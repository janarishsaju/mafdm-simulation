"""
backend/algorithm/bounded_confidence.py
========================================
Pure Bounded Confidence model — no LLM, no memory, no SIR dampening.

Leaders and followers update opinions solely through the CA neighbourhood
sum, gated by a fixed ε interaction threshold.  No OpenAI calls are made.

Leader update:
    T_ij = R * sqrt(|O_j|) * (O_j - O_i)   if |O_j - O_i| <= eps  else 0
    O_new = clip(R * O_i + W * sum(T_ij), -1, 1)

Follower update (naked — no SIR):
    influence = sum((O_j - O_i) * sqrt(N) * |O_j|)  for |O_j - O_i| <= eps
    O_new = clip(R * O_i + W * influence, -1, 1)
"""
...
class BoundedConfidence(AbstractAlgorithm):
    ...
    # ── Leader update — pure CA, no LLM blend ────────────────────────────────
    ...
    # ── Follower update — pure CA, no SIR dampening ──────────────────────────
    ...
    def update_follower(
        ...
        return float(np.clip(R * current_opinion + W * influence, -1.0, 1.0))
